...
- PPPoE подключения (Ростелеком, Дом.ру и др.) на Win 7/8 требуют ручной настройки прокси через Свойства браузера → Подключения → Настройки конкретного подключения (реестровый прокси работает только для LAN)
